...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    .querySelector("#child")
    .addEventListener("click", async () => filterCrochets("Child"));
  document
    .querySelector("#adult")
    .addEventListener("click", async () => filterCrochets("Adult"));
  document
    .querySelector("#granny")
    .addEventListener("click", async () => filterCrochets("Granny"));
});

async function filterCrochets(level) {
  const response = await fetch(url);
  const data = await response.json();
  const filteredCrochets = data.crochets.filter(
    (crochet) => crochet.level.toLowerCase() === level.toLowerCase(),
  );
...
const membershipData = {
  child: {
    ...
    description:
      "Perfect for those new to crochet. Includes access to beginner tutorials, basic patterns, and a supportive community.",
  },
  adult: {
    title: "Intermediate Crochet Membership",
    description:
      "Designed for those with some experience. Gain access to advanced patterns, exclusive workshops, and skill-building exercises.",
  },
  granny: {
    title: "Advanced Crochet Membership",
    description:
      "For experienced crocheters. Enjoy masterclasses, complex patterns, personalized feedback, and professional networking opportunities.",
  },
};
...
function thisYear() {
  ...
  document.getElementById("currentyear").innerHTML =
    "&copy;" +
    d.getFullYear() +
    " | 👩🏼‍💻 Greice Moreira | Rio Grande do Sul, Brazil";
}
